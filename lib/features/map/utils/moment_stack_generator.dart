...
class MomentStackGenerator {
  static Future<Uint8List> generateStackedMomentMarker({
    ...
    final cardsToShow = moments.length > maxCardsToShow ? maxCardsToShow : moments.length;
    ...
    if (moments.length > 1) {
      _drawCountBadge(canvas, moments.length, cardWidth, cardHeight, stackOffset);
    }
    
    final picture = recorder.endRecording();
    final totalWidth = (cardWidth + (cardsToShow - 1) * stackOffset + 20).toInt();
    final totalHeight = (cardHeight + (cardsToShow - 1) * stackOffset + 20).toInt();
  ...
  static void _drawStackShadow(Canvas canvas, int cardCount, double width, double height, double offset) {
    final paint = Paint()
  ...
  static void _drawCountBadge(Canvas canvas, int count, double cardWidth, double cardHeight, double stackOffset) {
  ...
  static Future<ui.Image?> _loadNetworkImage(String url) async {
    try {
      ...
      print('Image loaded successfully: ${frame.image.width}x${frame.image.height}');
      return frame.image;
    } catch (e) {
